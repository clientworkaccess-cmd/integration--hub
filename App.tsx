...
import { Integration, AppState } from './types.ts';
import IntegrationCard from './components/IntegrationCard.tsx';
import EmailModal from './components/EmailModal.tsx';
import { GITHUB_AUTH_URL } from './constants.tsx';
import { triggerWebhook } from './services/webhookService.ts';
...
const App: React.FC = () => {
  ...
  const handleOAuthCallback = useCallback(async (code: string) => {
    ...
    if (success) {
      setState(prev => ({ ...prev, isConnecting: false, success: true, error: null }));
          ...
          <div className="flex justify-between h-16 items-center">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="w-10 h-10 bg-slate-200 rounded-full flex items-center justify-center border-2 border-white shadow-sm overflow-hidden">
                <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${state.email || 'guest'}`} alt="avatar" />
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 w-full">
        ...
        {state.error && (
          <div className="mb-8 p-4 bg-red-50 border border-red-200 rounded-xl flex items-center gap-3 text-red-700">
            ...
            <button onClick={() => setState(p => ({ ...p, error: null }))} className="ml-auto hover:bg-red-100 p-1 rounded-md">
        ...
        {state.success && (
          <div className="mb-8 p-4 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center gap-3 text-emerald-700">
            ...
            <button onClick={() => setState(p => ({ ...p, success: false }))} className="ml-auto hover:bg-emerald-100 p-1 rounded-md">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
          </div>
        )}
